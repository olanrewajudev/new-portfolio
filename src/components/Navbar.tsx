...
export default function Navbar() {
    return (
        <nav className="bg-white/25 backdrop-blur-2xl  shadow-2xl rounded-full px-8 sm:py-6 fixed w-[90%] mx-auto top-1 left-1/2 -translate-x-1/2 flex items-center justify-between z-50">
            <div className="bg-clip-text text-transparent bg-gradient-to-r from-yellow to-primary lg:text-3xl text-2xl font-bold">Atolagbe Olanrewaju.</div>
            <div className="hidden md:block">
                {navItem.map((_item, i: number) => (
                    <a href={_item.href} className="px-5 font-semibold text-yellow" key={i}>{_item.name}</a>
                ))}
            </div>
            <div className="lg:hidden flex text-yellow text-xl items-center"><SlMenu /></div>
